...
def open_application(app_name_or_path):
    """Opens an application on Windows using win32api.ShellExecute, with improved name resolution."""
    common_apps = {
        "notepad": "notepad.exe",
        "chrome": "chrome.exe",
        "firefox": "firefox.exe",
    ...
    # If no extension and not in common apps, try appending .exe
    elif "." not in target_app and not os.path.exists(target_app):
        app_to_open = target_app + ".exe"
    else:
        app_to_open = app_name_or_path # Use as is

    print(f"Attempting to open: {app_to_open}") # Debug print
    try:
        win32api.ShellExecute(0, "open", app_to_open, None, None, win32con.SW_SHOWNORMAL)
        return f"Successfully attempted to open '{app_name_or_path}'. Check your desktop or taskbar."
    except win32api.error as e:
        return f"Error opening '{app_name_or_path}': {e.strerror}. Please ensure the application name or path is correct and accessible."
    except Exception as e:
        return f"An unexpected error occurred while trying to open '{app_name_or_path}': {e}. Please ensure the application name or path is correct."
...
def close_application(app_name):
    ...
    if closed_count > 0:
        return f"Successfully closed {closed_count} instance(s) of '{app_name}'. (Requires exact process image name, e.g., 'chrome.exe')"
    else:
        return f"Could not find or close any instance of '{app_name}'. Please ensure you provide the exact process image name (e.g., 'chrome.exe')."
...
def clean_temp_files():
    ...
    for item in os.listdir(temp_dir):
        ...
        try:
            ...
            errors += 1
    return f"Cleaned {cleaned_files} items from the temporary directory. Could not clean {errors} items."
...
def list_processes():
    ...
    try:
        ...
        return f"Error listing processes: {e.stderr.strip()}"
    except Exception as e:
...
def get_help_message():
    ...
    help_msg = "NOVA Commands:\n"
    help_msg += "  - 'open application [app_name_or_path]': Opens an application. You can use common names (e.g., 'notepad', 'chrome') or the full path to the executable (e.g., 'C:\\Program Files\\App\\app.exe').\n"
...
class ChatbotGUI:
    def __init__(self, master):
        ...
        self.chat_history.insert(tk.END, "NOVA: Hello! I'm NOVA, an AI built by spiderboy using the gemma llm model.\n")
    ...
    def send_message(self, event=None):
        ...
        if user_input.lower().startswith("open application "):
