...
# Add parent directory to path for nova_system imports
...
# MCP Agent support
try:
    from agent.claude_mcp_agent import ClaudeMCPAgent as MCPAgent
...
# Voice Control (from Jarvis-AI)
# NOVA VOICE ENGINE - Using Soprano TTS
TTS_ENGINE = None
VOICE_AVAILABLE = True # Enable voice via Nova Voice Engine

# Try to import Nova Voice (Soprano TTS + Whisper STT)
try:
    from nova_system.tts import get_nova_voice, nova_say, nova_listen, SOPRANO_AVAILABLE
...
# TTS function using Nova Voice (Soprano)
def nova_speak(text):
    """Speak text using Soprano TTS."""
    if NOVA_VOICE_AVAILABLE:
        try:
            return nova_say(text)
        except Exception as e:
...
class AppFinder:
    ...
    CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "app_cache.json")
...
# ============= VOICE CONTROL (Soprano TTS + Whisper STT) =============
# Nova Voice singleton
_nova_voice_instance = None

def get_voice_engine():
    """Get or create Nova Voice engine with Soprano TTS and Whisper STT."""
    global _nova_voice_instance
    if _nova_voice_instance is None:
        if NOVA_VOICE_AVAILABLE:
            try:
                _nova_voice_instance = get_nova_voice(device="auto")
                print("✅ Nova Voice connected (Soprano TTS + Whisper STT)")
            except Exception as e:
                print(f"⚠️ Voice init failed: {e}")
                _nova_voice_instance = None
    return _nova_voice_instance

class VoiceControl:
    """
    Voice control for Nova - text-to-speech and speech recognition.
    Enables hands-free operation like Jarvis.
    TTS: Soprano (ekwek/Soprano-1.1-80M)
    ...
    def speak(text, force=False):
        ...
        if not VOICE_AVAILABLE:
            print("⚠️ Voice not available (VOICE_AVAILABLE=False)")
            return False
        if not VOICE_REPLY_ENABLED and not force:
            return False
        
        try:
            # Use Soprano TTS (no fallback to Zira)
            if NOVA_VOICE_AVAILABLE:
                voice = get_voice_engine()
                if voice and voice.tts:
                    return voice.say(text)
                else:
                    print("⚠️ Voice engine not initialized properly")
            else:
                print("⚠️ Nova Voice module not available")
            return False
        except Exception as e:
            print(f"⚠️ TTS error: {e}")
            return False
    ...
    def listen(timeout=5):
        """Listen for voice input using Whisper STT."""
        # Try using Nova Voice's Whisper STT
        if NOVA_VOICE_AVAILABLE:
            try:
                voice = get_voice_engine()
                if voice and voice.stt:
                    return voice.listen(timeout=timeout)
            except Exception as e:
                print(f"⚠️ Whisper STT error: {e}")
        
        # Fallback to speech_recognition with Whisper
        ...
        try:
            ...
            try:
                import whisper
                wav_path = str(Path(tempfile.gettempdir()) / "nova_whisper_input.wav")
                with open(wav_path, "wb") as f:
                    f.write(audio.get_wav_data())
                
                # Load or use cached model
                global whisper_model_cache
                if 'whisper_model_cache' not in globals() or whisper_model_cache is None:
                    whisper_model_cache = whisper.load_model("base")
                
                result = whisper_model_cache.transcribe(wav_path, fp16=False)
    ...
    def listen_for_wake_word(wake_word="nova"):
        """Listen continuously for the wake word using Whisper."""
        try:
            text = VoiceControl.listen(timeout=3)
            if text and wake_word.lower() in text:
                VoiceControl.speak("Yes sir, how can I help?", force=True)
                return True
        except:
            pass
        return False
...
=== SYSTEM ===
[TOOL:open_app] app
  Example: [TOOL:open_app] chrome
  Example: [TOOL:open_app] notepad
  Example: [TOOL:open_app] vscode

[TOOL:close_app] app
  Example: [TOOL:close_app] chrome
  Example: [TOOL:close_app] notepad

[TOOL:system_status]
[TOOL:lock_screen]

CRITICAL: When user says "open X", "launch X", "start X", ALWAYS respond with [TOOL:open_app] X
CRITICAL: When user says "close X", "exit X", "kill X", ALWAYS respond with [TOOL:close_app] X
    ...
    def _execute_ollama_tool(self, tool_name: str, args: str) -> str:
        ...
        try:
            ...
            if tool_name == "search_google":
                ...
                result = SystemControl.open_app(args)
                return result
            
            elif tool_name == "close_app":
                result = SystemControl.close_app(args)
                return result
...
"""
    ...
    def _process_tool_calls(self, response):
        ...
        json_tool_matches = re.findall(r'```tool\s*\n?({.*?})\s*\n?```', response, re.DOTALL)
        for tool_json in json_tool_matches:
            try:
                ...
                if tool_name == "open_app":
                    app = args.get("app", "")
                    print(f"🚀 [TOOL] Opening app: {app}...")
                    result = SystemControl.open_app(app)
                    print(f"✅ {result}")
                    
                elif tool_name == "close_app":
                    app = args.get("app", "")
                    print(f"🛑 [TOOL] Closing app: {app}...")
                    result = SystemControl.close_app(app)
                    print(f"✅ {result}")
                    ...
                    url = f"https://www.google.com/search?q={query.replace(' ', '+')}"
                    webbrowser.open(url)
                    print(f"🔍 [TOOL] Searched Google for: {query}")
                    
                elif tool_name == "system_status":
                    status = SystemControl.get_system_status()
                    print(f"📊 [TOOL] System Status:")
                    print(f"   CPU: {status.get('cpu_percent')}%")
                    print(f"   Memory: {status.get('memory_percent')}%")
                    print(f"   Battery: {status.get('battery_percent', 'N/A')}%")
                    
                elif tool_name == "create_python_file":
                    code = args.get("code", "")
                    filename = args.get("filename", "temp.py")
                    filepath = os.path.join(workspace, filename)
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(code)
                    print(f"📄 [TOOL] Created: {filepath}")
                    
                elif tool_name == "execute_python_file":
        ...
        # Detect OPEN_APP (for opening applications)
        open_app_matches = re.findall(r'<OPEN_APP name="([^"]+)"\s*/>', response)
        for app_name in open_app_matches:
            print(f"🚀 [TOOL] Opening app: {app_name}...")
            result = SystemControl.open_app(app_name)
            print(f"✅ {result}")
        
        # Also check for [TOOL:open_app] format
        tool_open_matches = re.findall(r'\[TOOL:open_app\]\s*(\w+)', response)
        for app_name in tool_open_matches:
            print(f"🚀 [TOOL] Opening app: {app_name}...")
            result = SystemControl.open_app(app_name)
            print(f"✅ {result}")
        
        # Detect CLOSE_APP
        close_app_matches = re.findall(r'<CLOSE_APP name="([^"]+)"\s*/>', response)
        for app_name in close_app_matches:
            print(f"🛑 [TOOL] Closing app: {app_name}...")
            result = SystemControl.close_app(app_name)
            print(f"✅ {result}")
        
        # Also check for [TOOL:close_app] format
        tool_close_matches = re.findall(r'\[TOOL:close_app\]\s*(\w+)', response)
        for app_name in tool_close_matches:
            print(f"🛑 [TOOL] Closing app: {app_name}...")
            result = SystemControl.close_app(app_name)
            print(f"✅ {result}")
        
        # Detect natural language patterns for app control
        lower_response = response.lower()
        if "can't open" in lower_response or "cannot open" in lower_response or "unable to open" in lower_response:
            # LLM refused to open - let's try to extract what they wanted to open
            app_patterns = ['chrome', 'notepad', 'firefox', 'edge', 'word', 'excel', 'vscode', 'spotify', 'vlc', 'calculator', 'paint']
            for app in app_patterns:
                if app in lower_response:
                    print(f"🔄 [OVERRIDE] Nova said it can't open {app}, but I can! Opening...")
                    result = SystemControl.open_app(app)
                    print(f"✅ {result}")
                    break
  ...
  exit                - Exit automation mode
    ...
    def start_gui_mode(self):
        ...
        try:
            ...
            # Kill any existing instances of gui.py to prevent conflicts
            if PSUTIL_AVAILABLE:
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        if proc.info['cmdline'] and 'gui.py' in ' '.join(proc.info['cmdline']):
                            proc.kill()
                    except:
                        pass

            # Check dependencies (GUI)
            try:
                import importlib.util
                if not importlib.util.find_spec("PyQt6") or not importlib.util.find_spec("PyQt6.QtWebEngineWidgets"):
                    raise ImportError
            except ImportError:
                if RICH_AVAILABLE and console:
                    console.print("  [bold yellow]⚠️  GUI libraries missing. Installing PyQt6...[/]")
                else:
                    print("  ⚠️  GUI libraries missing. Installing PyQt6...")
                ...
                try:
                    ...
                    if RICH_AVAILABLE and console:
                        console.print(f"  [bold red]❌ Failed to install dependencies: {e}[/]")
                    else:
                        print(f"  ❌ Failed to install dependencies: {e}")
                    return

            # Check dependencies (Voice)
            try:
                import speech_recognition
            except ImportError:
                if RICH_AVAILABLE and console:
                    console.print("  [bold yellow]⚠️  Voice libraries missing. Installing SpeechRecognition...[/]")
                else:
                    print("  ⚠️  Voice libraries missing. Installing SpeechRecognition...")
                
                try:
                    subprocess.run([sys.executable, "-m", "pip", "install", "SpeechRecognition", "pyaudio"], 
                                   check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    if RICH_AVAILABLE and console:
                        console.print("  [bold green]✅ Voice dependencies installed![/]")
                except Exception as e:
                     pass # Continue without voice if it fails (pyaudio is hard to install sometimes)
                ...
                if user_input.startswith("/"):
                    ...
                    if cmd in ["/exit", "/quit", "/q"]:
                        ...
                        if SR_AVAILABLE:
                            ...
                            # Pre-initialize voice engine
                            print("\n  🔄 Initializing voice engine...")
                            voice_engine = get_voice_engine()
                            if not voice_engine:
                                print("  ⚠️ Voice engine failed to initialize.")
                                print(f"  NOVA_VOICE_AVAILABLE: {NOVA_VOICE_AVAILABLE}")
                                print("  Try: pip install edge-tts pyttsx3 openai-whisper")
                                # Try direct initialization
                                try:
                                    from nova_system.tts import NovaVoice
                                    voice_engine = NovaVoice()
                            ...
                            # Pre-load computer control ONCE (faster loop)
                            try:
                                from nova_system.computer_control import nova_execute
                                COMPUTER_CONTROL = True
                            except ImportError:
                                COMPUTER_CONTROL = False
                                nova_execute = None
                                
                            if VOICE_AVAILABLE and voice_engine:
                                VoiceControl.speak("Hello sir, Nova at your service.", force=True)
                                time.sleep(0.5)  # Wait for audio playback to finish
                            
                            # Voice loop - process commands inline
                            voice_active = True
                            while voice_active:
                                print("\n  🎤 Listening...")
                                v_input = VoiceControl.listen(timeout=10)
                                if not v_input:
                                    print("  (No speech detected, listening again...)")
                                    continue
                                    
                                lower_voice = v_input.lower().strip()
                                
                                if 'exit' in lower_voice or 'stop' in lower_voice or 'quit' in lower_voice or 'deactivate' in lower_voice:
                                    print("  🔇 Voice mode deactivated.")
                                    if VOICE_AVAILABLE:
                                        VoiceControl.speak("Voice mode deactivated, sir.", force=True)
                                    voice_active = False
                                    break
                                
                                print(f"\n  📝 Command: {v_input}")
                                
                                # ===== TRY COMPUTER CONTROL FIRST =====
                                if COMPUTER_CONTROL and nova_execute:
                                    success, response = nova_execute(lower_voice)
                                ...
                                # ===== CONVERSATIONAL =====
                                if lower_voice in ['hello', 'hi', 'hey', 'hello nova', 'hi nova']:
                                ...
                                # ===== FALLBACK TO NOVA BRAIN =====
                                print("  ⏳ Asking Nova...")
                                try:
                                    if self.brain and hasattr(self.brain, 'chat'):
                                        response = self.brain.chat(v_input)
                                    elif self.brain and hasattr(self.brain, 'process'):
                                        response = self.brain.process(v_input)
                                    elif self.nova:
                                        response = self.nova.process(v_input)
                                    else:
                                    ...
                                    if response:
                                        print(f"\n  🤖 Nova: {response}")
                                        if VOICE_AVAILABLE:
                                            VoiceControl.speak(response[:300], force=True)
                                except Exception as e:
                                    print(f"  ⚠️ Error: {e}")
                            ...
                            while voice_mode_active:
                                ...
                                if voice_input:
                                    if 'exit' in voice_input or 'stop' in voice_input or 'quit' in voice_input or 'deactivate' in voice_input:
                                        print("  🔇 Voice mode deactivated.\n")
                                        if VOICE_AVAILABLE:
                                            VoiceControl.speak("Goodbye sir, voice mode deactivated.", force=True)
                                    ...
                                    if lower_voice in ['hello', 'hi', 'hey', 'hello nova', 'hi nova', 'hey nova']:
                                        ...
                                        response = "I'm doing great sir, thank you for asking! Ready to assist."
                                        print(f"  🗣️ {response}")
                                        if VOICE_AVAILABLE:
                                            VoiceControl.speak(response, force=True)
                                        ...
                                        response = "I can open any app, search Google, play YouTube videos, control volume, lock your computer, and much more. Just tell me what you need, sir."
                                        print(f"  🗣️ {response}")
                                        if VOICE_AVAILABLE:
                                            VoiceControl.speak(response, force=True)
